...
export async function generateStaticParams() {
  if (process.env.BUILD_TARGET !== 'gigantec') {
  ...
  for (const linha of linhas) {
    ...
    try {
      ...
      if (obj.slug) {
        const produto = await lerProdutoPorSlug(obj.slug, 'GIGANTEC', lote);
        if (produto) {
...
export async function generateMetadata({ params }) {
  const lote = process.env.LOTE || null;
  const produto = await lerProdutoPorSlug(params.slug, 'GIGANTEC', lote);
  if (!produto) return {};
