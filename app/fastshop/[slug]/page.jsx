...
export async function generateStaticParams() {
  if (process.env.BUILD_TARGET !== 'fastshop') {
  ...
  for (const linha of linhas) {
    ...
    try {
      ...
      if (obj.slug) {
        const produto = await lerProdutoPorSlug(obj.slug, 'FASTSHOP', lote);
        if (produto) {
...
export async function generateMetadata({ params }) {
  const lote = process.env.LOTE || null;
  const produto = await lerProdutoPorSlug(params.slug, 'FASTSHOP', lote);
  if (!produto) return {};
